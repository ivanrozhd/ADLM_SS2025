...
def text2image_ldm_stable(
    ...
    # Prepares prompts and gets embeddings
...
# Hooking into the Cross-Attention Layers
def register_attention_control(model, controller):
    """
    Traverse the UNet inside the Stable Diffusion model. 
    Find all CrossAttention layers. And override their forward() function with a new one (ca_forward()), which includes a hook to the custom controller.
    ...
    class DummyController:
        # simply returns the attention unchanged
    ...
    # Recursively walk through the model and replaces the forward() of CrossAttention modules with ca_forward()
    def register_recr(net_, count, place_in_unet):
        #if net_.__class__.__name__ == 'CrossAttention': #Ngoc changed
        if net_.__class__.__name__ == 'Attention': #Ngoc changed
            #print("NGOC CHANGED TO ATTENTION")
    ...
    for net in sub_nets:
        if "down" in net[0]: # UNet downsampling blocks
            cross_att_count += register_recr(net[1], 0, "down")
        elif "up" in net[0]: # UNet upsampling blocks
            cross_att_count += register_recr(net[1], 0, "up")
        elif "mid" in net[0]: # UNet middle block (bottleneck)
